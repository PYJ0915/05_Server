...
-> 아무 속성도 없음!
	...
	<%-- 
		...
		1) null인 경우 true
		2) 연산 대상이 배열이거나 컬렉션인데 내부에 아무 요소도 없다면 true
		3) 나머지는 false
	 --%>
		<c:when test="${empty param.age}"> <%-- == ${$param.age == null} --%>
			<h4>age 값이 없습니다.</h4>
		</c:when>
...
6) varStatus : 현재 반복 상태와 관련된 정보를 제공하는 변수 선언
  ...
  * varStatus에서 제공되는 값
  ...
  - last : 마지막 반복이면 true, 아니면 false
 	...
 	<h3>5-2) 일반 for문 + 컬렉션 List 사용</h3>
 	
 	<ul>
 		<li>nameList : ${nameList}</li>
 		<li>
 			<%-- JSTL 접두사 fn이 추가된 EL 구문 사용 
 				fn(function, 기능) : 문자열, 컬렉션 관련 함수 제공하는 JSTL-EL
 		...
 		<c:forEach var="name" items="${nameList}" varStatus="vs">
 			...
 			<li>
 				<ul>
 					...
 					<li>현재 요소1 : ${vs.current}</li>
 					<li>현재 요소2 : ${nameList[vs.index]}</li>
 					<li>현재 요소3 : ${name}</li>
 				</ul>
 			</li>
