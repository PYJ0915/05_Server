...
// 클라이언트로부터 "/example1"이라는 요청을 받아서 처리한 후 응답을 해줄 서블릿 클래스 ExampleController1을 만든다

// Controller : 요청(Request)에 따라 어떤 service를 호출할 지 "제어"하고, 어떻게 응답(Response)할 지 "제어"하는 역할

public class ExampleController1 extends HttpServlet{ 
	
	// doGet() 메서드
	// => GET방식 요청을 처리하는 메서드
	// HTTPServelet을 상속받으면 사용할 수 있는 메서드로 오버라이딩 하여 사용
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// HttpServletRequest
		// - 클라이언트 요청 시 생성되는 객체
		// - 요청 시 전달된 데이터, 요청한 클라이언트 정보, 유연한 요청 처리를 하기 위한 객체를 제공
		
		// HttpServletResponse
		// - 클라이언트 요청 시 생성되는 객체
		// - 서버가 클라이언트에게 응답하기위한 방법을 제공
		// 서버 -> 클라이언트 쪽으로 만들어진 페이지를 제공하기 위해서 출력용 스트림 제공(PrintWriter)
		
		System.out.println("--- 이름, 나이를 입력받아 처리하는 Servlet 코드 ---");
		...
		// 2) 응답 받은 내용을 클라이언트의 브라우저가 해석하여 화면에 출력해줌
		
		// HttpServletResponse 객체 이용
		// 응답처리 1 : 응답하는 문서의 형식과 문자 인코딩 지정
		resp.setContentType("text/html; charset=UTF-8");
		
		// 응답처리 2 : 서버 -> 클라이언트 연결되는 출력용 스트림 얻어오기
		PrintWriter out = resp.getWriter();
		
		// 응답처리 3 : 출력할 HTML코드 만들기
		...
		// 응답처리 4 : 출력스트림을 이용해 클라이언트에게 HTML 코드 제공
